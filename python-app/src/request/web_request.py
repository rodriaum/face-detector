...
from src.utils.config import CLIENT_TIMEOUT, APP_IDENTIFIER
...
class WebRequest:
    ...
    async def upload_image(self, image, filename='face.jpg'):
        ...
        form.add_field('uploadedBy', APP_IDENTIFIER)
    ...
    async def delete_image(self, image_id):
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.client_timeout) as session:
                async with session.delete(f'{self.api_url}/v1/images/delete/{image_id}', headers=self.headers, ssl=self.ssl_context) as response:
                    if response.status == 200:
    ...
    async def get_all_images(self, colour_mode_id=None):
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.client_timeout) as session:
                async with session.get(f'{self.api_url}/v1/images/get-all', headers=self.headers, ssl=self.ssl_context) as response:
                    if response.status == 200:
                        ...
                        try:
                            ...
                            if 'error' in error_json:
                                print(f"Motive: {error_json['error']}")
                            else:
                                print(f"Motive: {error_json}")

                        except Exception as e:
                            print(f"Failed to parse error response JSON: {e}")
    ...
    async def ping_connection(self):
        try:
            print(f"Checking connection at {self.api_url}")

            async with aiohttp.ClientSession(timeout=self.client_timeout) as session:
                async with session.get(f'{self.api_url}/v1/info/ping',
                    ...
                    if response.status == 200:
                        ...
                        if message:
                            print(f"API is reachable. ({message})")
                        else:
